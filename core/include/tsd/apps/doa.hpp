...
namespace tsd::apps::doa {
  ...
  extern Vecf musicdoa_1d(const Tabcf &R, float d, entier Ns);
   ...
   */
  extern Tabcf steervec_1d(const Vecf &pos, const Vecf &angles);


  /** @brief Calcul de la matrice de covariance théorique.
   *
   *  Calcul de la matrice de covariance théorique pour un réseau de micros / antennes
   *  linéaires 1d et équidistantes.
   *
   *  @param pos    Position des antennes (unité = longueur d'onde de la porteuse)
   *  @param angles Angles d'incidences des différentes sources (en radians)
   ...
   */
  extern Tabcf sensorcov_1d(const Vecf &pos, const Vecf &angles, float SNR_db);
