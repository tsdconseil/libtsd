 ...
 *  \~english @brief Signal transformations: FFT, CQT, ... */
 ...
 */
  ...
  /** @cond undoc */
  extern std::function<sptr<FFTPlanAbstrait>()> fftplan_defaut;
  /** @endcond */
 ...
 * @param normaliser  Si vrai, alors la transformée inclut un facteur de normalisation en 1/racine(N) de manière à préserver l'énergie.
 * @return Un @ref Filtre cfloat vers cfloat.
 *
 ...
 *
 * Notez que si vos échantillons d'entrée sont réels, alors un plan plus efficace existe : @ref creation_rfft_plan().
 *
 * @par Exemple
 * @code
 * int N = 1024; // Dimension des blocs
 * auto plan = creation_fft_plan();
 * for(i = 0; i < n; i++)
 * {
 ...
 * @endcode
 *
 * @sa creation_rfft_plan(), fft(), ifft()
 **/
  extern sptr<FFTPlanAbstrait> creation_fft_plan(int n = -1, bool avant = true, bool normaliser = true);
 ...
 *
 * @param n       Dimension des vecteurs (paramètre optionnel)
 * @return Un @ref Filtre float vers cfloat.
 *
 * Le paramètre n n'a pas besoin d'être précisé, et la dimension des vecteurs peut changer en cours de route, si c'est le cas, les facteurs de rotations sont automatiquement réévalués.
 *
 *
 ...
 * @endcode
 *
 * @sa creation_fft_plan(), fft(), ifft()
 */
extern sptr<FiltreGen<float, cfloat>> creation_rfft_plan(int n);


/** @brief Calcul de la FFT d'un signal réel
 *
 *  <h3>FFT d'un signal réel</h3>
 *
 *  Cette fonction calcule de manière efficace la FFT d'un signal réel <b>de dimension paire</b>
 *  (si le signal fourni comporte un nombre impair d'éléments, la routine standard de la FFT est appelée).
 *
...
ArrayXcf rfft(const Eigen::ArrayBase<derived> &x)
{
  ...
  auto plan = tsd::fourier::creation_rfft_plan(x.rows());
  return plan->step(x);
}
 ...
 *
 * Notez que, du fait des hypothèses sous-jacentes à la TFD, des artefacts peuvent apparaitrent au bord du signal.
 ...
 *
 *  Cette fonction calcule la TFD <b>normalisée</b> d'un signal réel ou complexe :
 *  @f[
 *    X_n = \frac{1}{\sqrt{N}}\cdot\sum_{k=0}^{N-1} x_k \cdot e^{\frac{-2\pi\mathbf{i}kn}{N}}
 *  @f]
 *
 *  @param x Signal dans le domaine temporel
 *  @returns X = TFD(x)
 *
 *  @par Exemple
 *  @code
 ...
 *
 *  @sa rfft(), ifft()
 *  @note Pour un signal réel, la fonction rfft, plus rapide, est recommandée. */
template<typename derived>
auto fft(const Eigen::ArrayBase<derived> &x)
{
  if constexpr(est_complexe<typename derived::Scalar>())
  {
    auto plan = tsd::fourier::creation_fft_plan(x.rows());
    ArrayXcf x2 = x, y;
    plan->step(x2, y, true);
    return y;
  }
...
{
  auto plan = tsd::fourier::creation_fft_plan(X.rows(), false);
  ArrayXcf X2 = X, Y;
 ...
 *
 *  Ecrase la deuxième partie du vecteur (ainsi que la partie imaginaire du premier élément,
...
template<typename T = float>
  Vecteur<T> delais(const Vecteur<T> &x, float tau);
...
extern std::ostream &operator <<(std::ostream &os, const DetecteurConfig::Detection &det);
